...
% 1) Blist: the matrix containing the screw axes expressed in the body
% frame as its columns.
% 2) M: the zero-position home config. of end-effector expressed in
% the space frame.
% 3) T: the desired end-effector config.  expressed in the space frame.
...
function [thetalist, success] = IKinBodyIterates(Blist, M, T, thetalist0, eomg, ev)
  ...
  thetaMat(1,:)=transpose(thetalist0); % the first row of thetaMat contains the initial guess. 
  i = 0;
  ...
  while err && i < maxiterations % iterate the following while the error is larger than the specified input value.
      ...
      fprintf('SE(3) end effector configuration(WRT space frame): \n \n');
      disp(Ti); 
  ...
  fprintf('Joint matrix size is %4d X %4d  \n',size(thetaMat));
  fprintf('Rows(representing iterations)  Cols(representing joint variables).');
  % finally, save the iteration matrix (named iterates.csv) in the current MATLAB workspace
  csvwrite('iterations.csv',thetaMat);
  success = ~ err;
end
